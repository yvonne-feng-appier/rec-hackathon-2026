...
func TestVendorPost(t *testing.T) {
	tt := []struct {
		...
		{
			name:  "GIVEN valid JSON without rCode THEN return the expected struct",
			input: []byte(`{"rCode":"","rMessage":"","data":{"result":[{"productId":1,"productUrl":"url1","productImage":"img1"}]}}`),
		...
		{
			name:        "GIVEN empty result array THEN return an error",
